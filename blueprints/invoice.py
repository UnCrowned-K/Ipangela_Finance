...
def _invoice_storage_dir() -> str:
    """Return a per-user invoice storage directory."""
    root = current_app.config.get('INVOICE_STORAGE_FOLDER',
                                  os.path.join(_APP_ROOT, 'invoices'))
...
def save_invoice():
    ...
    try:
        ...
        invoice_core = InvoiceCore(_invoice_storage_dir())
...
def send_invoice_email():
    ...
    try:
        ...
        from invoice_core import Currency
        ...
        class MinimalInvoice:
            def __init__(self, data):
                ...
                try:
                    self.currency_info = Currency[self.currency].value
                except KeyError:
                    self.currency_info = Currency.USD.value
                self.business_name = data.get('business', {}).get('name', '')
                self.issue_date = data.get('issue_date', '')
                self.due_date = data.get('due_date', '')
                self.grand_total = data.get('totals', {}).get('grand_total', 0) or 0
